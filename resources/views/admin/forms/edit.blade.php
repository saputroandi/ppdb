...
@section('content')
<form action="/forms/{{$user->id}}" method="post">
    {{ csrf_field() }}
    @method('PATCH')
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <input type="text" class="form-control" id="InputInterest" name="majors_interest"
                value="{{$user->majors_interest}}">
            <small style="color: red;">@error('majors_interest'){{$message}}@enderror</small>
        </div>
    </div>
    <div class="form-group row">
        <div class="col-sm-2"><a href="/forms/{{$user->id}}" class="btn btn-secondary">Back</a></div>
